  ...
  <tr><td>
    <?php
     ...
     foreach ($pdo->query($sql) as $sprechwunsch) {
       if($sprechwunsch['funk_neu'] == "1"){
         echo "<embed type=\"audio/mp3\" src=\"fms5.mp3\" width=\"0\" height=\"0\" >";
         $ssl = "UPDATE funk_schlange SET funk_neu = ? WHERE funk_id = ?";
         $isset = $pdo->prepare('UPDATE funk_schlange SET funk_neu = ? WHERE funk_id = ?');
         $a = $sprechwunsch['funk_von'];
         $isset->execute(["",$a]);
        }
       if($sprechwunsch['funk_warten'] != 1){
         if($sprechwunsch['funk_status'] == 5){

       $nr = $sprechwunsch['funk_von'];
       $help = "SELECT * FROM fahrzeuge WHERE fahrzeug_id = ". $nr ;
       ?>  <tr><td style="background-color: green;"><?php
       foreach ($pdo->query($help) as $em) {?>
         <div style="background-color: green;"><?php
       ...
       <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">
         ...
         <input type="submit" name="f_sprechwunsch_warten" value="WARTEN" /><br />
